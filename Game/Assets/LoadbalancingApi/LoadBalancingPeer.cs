...
namespace ExitGames.Client.Photon.LoadBalancing
{
    ...
    public class LoadBalancingPeer : PhotonPeer
    {
        ...
        {
            return OpCreateRoom(roomName, isVisible, isOpen, maxPlayers, customGameProperties, propsListedInLobby, playerProperties, lobbyName, lobbyType, 0, 0);
        }
        ...
        /// <param name="lobbyName">Name of the lobby this game gets added to. Default: null, attached to default lobby. Lobbies are unique per lobbyName plus lobbyType, so the same name can be used when several types are existing.</param>
        /// <param name="lobbyType">Type of the (named)lobby this game gets added to.</param>
        /// <param name="playerTtl"></param>
        /// <param name="emptyRoomTtl"></param>
        /// <returns>If the operation could be sent (requires connection).</returns>
        public virtual bool OpCreateRoom(string roomName, bool isVisible, bool isOpen, byte maxPlayers, Hashtable customGameProperties, string[] propsListedInLobby, Hashtable playerProperties, string lobbyName, LobbyType lobbyType, int playerTtl, int emptyRoomTtl)
        {
            if (this.DebugOut >= DebugLevel.INFO)
            {
            ...
            if (playerTtl > 0)
            {
                op[ParameterCode.PlayerTTL] = playerTtl;   //TURNBASED
            }
            if (emptyRoomTtl > 0)
            {
                op[ParameterCode.EmptyRoomTTL] = emptyRoomTtl;   //TURNBASED
            }
        ...
        public virtual bool OpJoinRoom(string roomName, Hashtable playerProperties, int actorId, bool createIfNotExists)
        {
            ...
            if (createIfNotExists || actorId > 0)   //TURNBASED (workaround for some server issue we will fix)
            {
        ...
        {
            return OpAuthenticate(appId, appVersion, null, null, null);
        }
    ...
    {
        public const byte TurnbasedLeave = 252;
        ...
        /// <summary>(252) When player left game unexpecable and playerTtl > 0 this evet is fired</summary>
        public const byte Disconnect = LiteEventCode.Disconnect;
    ...
    {
        public const byte UriPath = 209; // type: string
        public const byte RpcCallParams = 208; // type: Dictionary<string, object>

        public const byte RpcCallRetCode = 207; // byte
        public const byte RpcCallRetMessage = 206; // string
        public const byte RpcCallRetData = 205; // Dictionary<string, object>
