...
public class GameGUIInfo : MonoBehaviour {
	...
	void OnGUI(){
		...
		GUI.Label(new Rect(0, 2*Screen.height/32, Screen.width, Screen.height/32 ), string.Format("First {0} then {1} Turn ", GM.instance.TurnOrder[0], GM.instance.TurnOrder[1]), textColor);
		GUI.Label(new Rect(0, 3*Screen.height/32, Screen.width, Screen.height/32 ), string.Format("Is it my turn {0}", GM.instance.IsItMyTurn()), textColor);
		GUI.Label(new Rect(0, 4*Screen.height/32, Screen.width, Screen.height/32 ), string.Format("IsNextPlayersTurn: {0}", GM.instance.IsNextPlayersTurn()), textColor);
